...
vector<int> masks;
vector<vector<int>> masksupersets(1025);
unordered_set<int> mask_set;

void recursive_solve(int n, int mask)
{
    if(n==0)
    {
    ...
    if(n>=2)
    {
        recursive_solve(n-2,mask<<2);
    }
    if(n>=1)
    {
        recursive_solve(n-1,(mask<<1)|1);
    }
}
...
void formedges()
{
    for(int i=0;i<(1<<n);i++)
    {
        ...
        {
            if((i&masks[j]) == i)masksupersets[i].push_back(masks[j]);
        }
    }
}
...
void generate_masks()
{
    ...
    // cerr<<bitset<7>(masks[i])<<"#"<<endl;
    // cerr<<endl;

    // for(int i=0;i<=1024;i++)
    // {
    //     if(masksupersets[i].size())
    //     {
    //         cerr<<bitset<7>(i)<<"!"<<endl;
    //         for(auto x:masksupersets[i])
    //             cerr<<bitset<7>(x)<<endl;
    //         cerr<<endl;
    //     }
    // }
}

vector<vector<int>> dp(1001,vector<int>(1025,-1));

mint solve(int m_sub, int mask)
{
    if(dp[m_sub][mask] != -1)return dp[m_sub][mask];
    if(m_sub == 1)return dp[m_sub][mask] = (mask_set.count(mask));
    mint ans = 0;
    if(m_sub == m)
    {
        for(auto newmask:masks)
        {
            ans += solve(m_sub-1,newmask);
        }
    }
    else
    for(auto newmask:masksupersets[mask])
    {
        newmask ^= mask;
        ans += solve(m_sub-1,newmask);
    }
    // debug(bitset<7>(mask).to_string(),m_sub,ans);
    return dp[m_sub][mask] = ans.val;
}
