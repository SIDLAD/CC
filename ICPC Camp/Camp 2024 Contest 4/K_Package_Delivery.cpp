...
vector<vector<mint>> mult = {{0,1},{1,1}};
vector<vector<mint>> start = {{0,1}};
mint fibonacci(int n)
{
    if(n == 0)return 0;
    return (Mat(start) * Mat(mult).pow(n-1)).a[0][1];
}

vector<vector<mint>> mult1 = {{1,1},{5,1}};
vector<vector<mint>> mult2 = {{1,-1},{-5,1}};
vector<vector<mint>> freshstart = {{1,1}};
mint R1(int n)
{
    mint num = (Mat(freshstart) * Mat(mult1).pow(n+1)).a[0][1] - (Mat(freshstart) * Mat(mult2).pow(n+1)).a[0][1];
    num /= 2;
    if(n == 0)
    num -= mint(2).inv();
    else
...
int32_t main(){
    // debug(fibonacci(0),fibonacci(1),fibonacci(2),fibonacci(3),fibonacci(1e11));
    // debug(R1(0),R1(1),R1(2));
    ...
    mint two = 2;
    cout<<two[n+2]*(fibonacci(n+2) - 1) - two[n + 1] * n + 16 * R1(n-2)<<endl;
}