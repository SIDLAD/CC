...
struct segtree
{
    ...
    vector<T> tree;
    vector<V> lazy;
    T zero = {INF,0,-1}; // Change according to QUERY operation
    ...
    T combine(T a, T b)
    {
        ...
        if(a[0] < b[0])return a;
        if(b[0] < a[0])return b;
        if(a[0] == b[0])return {a[0],a[1] + b[1],a[2]};
    }
    ...
    void build(vector<U> &v, int id = 0, int segl = 0, int segr = -1)
    {
        ...
        if (segl == segr)
        {
            tree[id] = {v[segl],1,segl};  // Change according to MODIFY operation
            return;
        }
        int mid = (segl + segr) / 2;
    ...
    void propagate(int id, int segl, int segr)
    {
        ...
        if(segl != segr)
        {
            ...
            {
                tree[child][0] -= lazy[id];  // Change according to MODIFY operation
