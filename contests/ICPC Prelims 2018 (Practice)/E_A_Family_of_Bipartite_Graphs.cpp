...
{
    assert(n <= F_LIM);
    static mint fact[F_LIM + 1] = {-1};
    if(fact[0] == -1)
    {
        fact[0] = 1;
        for(int i=1;i<=F_LIM;i++)
        {
            fact[i] = fact[i-1] * i;
        }
    }
...
{
    if(n < r)return 0;
    return factorial(n) * factorial_inv(n-r) * factorial_inv(r);
}

int dp1[2501][2501];

void calculatedp1(int _n, int _m)
{
    if(dp1[_n][_m] != -1){return;}
    if(_n == 1 || _m == 1){dp1[_n][_m] = 1;return;}
    
    mint n = _n,m = _m;
    
    mint tot = (m*(m+1)/2)[n.val] - (m*(m-1)/2)[n.val];
    for(int seg = 1;seg<_m;seg++)
    {
        for(int n1 = 1;n1 <= _n;n1++)
        {
            calculatedp1(n1,seg);
            int rseg = _m - seg;
            int n2 = _n - n1;
            tot -= ncr(_n,n1) * (mint)dp1[n1][seg] * ((mint)(rseg*(rseg + 1)/2))[n2];
        }
    }
    dp1[_n][_m] = tot.val;
}
