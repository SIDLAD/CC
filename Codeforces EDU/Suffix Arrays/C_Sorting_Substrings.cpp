...
int32_t main(){
    ...
    auto compare = [&](pair<int,int> p1,pair<int,int> p2)
    {
        auto [l1,r1] = p1;
        auto [l2,r2] = p2;
        int lcp = sa.lcp(l1,l2);

        if(lcp >= min(r1 - l1 + 1,r2 - l2 + 1))
        {
            if ((r1 - l1 + 1) == (r2 - l2 + 1)) return (p1 < p2);
            return ((r1 - l1 + 1) < (r2 - l2 + 1));
        }
        else
        {
            return (sa.c[l1] < sa.c[l2]);
        }
    };
    ...
    for(int i=0;i<n;i++)cin>>ans[i].first >> ans[i].second,ans[i].first--,ans[i].second--;
