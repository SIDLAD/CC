...
int32_t main(){
    ...
    PersistentDynamicSegTree st(0,2e5 + 1);
    for(int i=0;i<n;i++){
        st.modify(a[i],i,i);
    }
    vector<int> versions{st.lastroot};
    while (q--)
    {
        int t;
        cin >> t;
        if (t == 1)
        {
            ...
            cin >> k >> i >> x;
            k--;i--;
            versions[k] = st.modify(x,i,i,versions[k]);
        }
        else if (t == 2)
        {
            int k, l, r;
        ...
        else
        {
            ...
            cin >> k;
            k--;
            versions.push_back(st.load_root(versions[k]));
        }
    }
    // cout<<st.timer<<endl;
}