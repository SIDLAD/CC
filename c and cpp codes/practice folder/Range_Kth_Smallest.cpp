...
struct PersistentDynamicSegTree {
	...
	PersistentDynamicSegTree(int l, int r) : minl(l), maxr(r) {tree.reserve(4e7),tree.push_back(SegNode());}
...
int32_t main(){
    ...
    while(m--)
    {
        ...
        debug(st.tree[versions[j]].freq - st.tree[versions[i-1]].freq, j - i + 1);
...
//     PersistentDynamicSegTree st(-shift,shift);
//     vector<int> versions(n + 1);
//     versions[0] = st.lastroot;
//     for(int i=1;i<=n;i++)versions[i] = st.modify(1,v[i - 1],v[i - 1]);
//     while(m--)
//     {
//         int i,j,k;
//         cin>>i>>j>>k;
//         i++,k++;
//         assert(st.tree[versions[j]].freq - st.tree[versions[i-1]].freq == j - i + 1);
//         cout<<st.query(k,versions[i - 1],versions[j]) + shift<<endl;
//     }
// }