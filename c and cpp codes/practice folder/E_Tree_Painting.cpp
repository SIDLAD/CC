...
int32_t main(){
    ...
    auto dfs1 = [&](auto self, int node, int p)->void
    {
        ...
        for(auto child: edges[node])if(child != p)
        {
            ...
            subanswer[node] += subanswer[child];
        }
        subanswer[node] += subsize[node];
    };
    dfs1(dfs1,1,-1);
    ...
    auto dfs2 = [&](auto self, int node, int p)->void
    {
        if(p == -1)
        {
            answer[node] = subanswer[node];
        }
        else
        {
            answer[node] = n - 2 * subsize[node] + answer[p];
        }
        for(auto child: edges[node])if(child != p)
        {
            self(self,child,node);
