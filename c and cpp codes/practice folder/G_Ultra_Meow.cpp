...
mint factinv(int b)
{
    static mint factorial_inverse[5001];
    if(factorial_inverse[0] != 1)
    {
        for(int i=0;i<=5000;i++)factorial_inverse[i] = 1/fact(i);
    }
    return factorial_inverse[b];
}
mint fact(int a)
{
    static mint factorial[5001];
    if(factorial[0] != 1)
    {
        factorial[0] = 1;
        factorial[1] = 1;
...
int32_t main(){
    ...
    for(TT++;T--;TT++)
    {
        ...
        for(int i=0;i<=n;i++)
        {
            for(int j=1;j<=i+1;j++)
            {
                mint x = (i+j)*nck(min(i+j-1,n),j-1)*nck(max(n-i-j,0ll),i-j+1);
                ans+= x;
            }
        }
