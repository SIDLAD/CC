...
mint fact(int x) {
    static mint f[(int)2e5 + 1];
    if(f[0] == 0) {
        f[0] = f[1] = 1;
        for(int i=2;i<=2e5;++i) f[i] = f[i-1] * i;
    }
    return f[x];
}

mint rfact(int x) {
    static mint rf[(int)2e5 + 1];
    if(rf[0] == 0) {
        for(int i=0;i<=2e5;++i) rf[i] = fact(i).inv();
    }
    return rf[x];
}
...
signed main(){
    ...
    for(int i=n-2;i>=0;--i) rc[i] = rc[i + 1] + (s[i] == ')');
    mint ans = 0;
    for(int i=0;i<n;++i) if(s[i] == '(') {
        ans += nck(lc[i] + rc[i] - 1, rc[i] - 1);
    }
    cout << ans << endl;
}