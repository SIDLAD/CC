...
int32_t main(){
    ...
    for(;T--;)
    {
        ...
        for(int b1=0;b1<10;b1++)for(int b2=0;b2<10;b2++)
        {
            ...
            for(int i=0;i<n;i++)
            {
                bool b1val = a[i] & (1ll<<b1), b2val = a[i] & (1ll<<b2);
                mint prevdp[2][2] = {};
                assert(prevdp[1][1] == (mint)0);
                swap(dp,prevdp);
                for(int c1=0;c1<2;c1++)for(int c2 = 0;c2<2;c2++)
                {
                    dp[c1^b1val][c2^b2val] += prevdp[c1][c2] * p[i] * tenkinv;
                    dp[c1][c2] += prevdp[c1][c2] * (tenk - p[i]) * tenkinv;
                }
            }
            ans += dp[1][1] * (1ll<<(b1 + b2));
        }
        cout<<ans<<endl;
    }
}