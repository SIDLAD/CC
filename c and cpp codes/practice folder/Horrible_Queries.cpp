...
struct segtree
{
    ...
    void propagate(int id, int segl, int segr)
    {
        ...
        if(segl != segr)
        {
            ...
            for(int i=0;i<2;i++)
            {
                int child = children[i],len = lengths[i];
                tree[child] += lazy[id]*len;  // Change according to MODIFY operation
    ...
    void modify(U val, int index_l, int index_r, int id = 0, int segl = 0, int segr = -1)
    {
        ...
        {
            tree[id] += val * (segr - segl + 1); // Change according to MODIFY operation
...
int32_t main(){
    ...
    for(;T--;)
    {
        ...
        while(c--)
        {
            ...
            else cout<<st.query(p-1,q-1)<<endl;
        }
    }
}