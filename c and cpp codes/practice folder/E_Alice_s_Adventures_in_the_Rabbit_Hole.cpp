...
int32_t main(){
    ...
    for(;T--;)
    {
        ...
        vector<int> distancetonearestleaf(n + 1,INF);
        vector<mint> sol(n + 1);
        sol[1] = 1;

        auto dfs1 = [&](auto self, int node, int p)->void
        {
            for(auto son:edges[node])if(son!=p)
            {
                self(self,son,node);
                distancetonearestleaf[node] = min(distancetonearestleaf[node],1 + distancetonearestleaf[son]);
            }
            if(distancetonearestleaf[node] == INF)distancetonearestleaf[node] = 0;
        };
        dfs1(dfs1,1,1);
        ...
        auto dfs2 = [&](auto self, int node, int p)->void
        {
            if(node != 1)
            {
                sol[node] = sol[p]*(1 - 1/(mint)(distancetonearestleaf[node] + 1));
            }
            for(auto son:edges[node])if(son != p)
            {
                self(self,son,node);
