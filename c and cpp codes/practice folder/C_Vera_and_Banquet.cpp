...
int32_t main(){
    ...
    string snew = s + s + '#' + sr + sr;
    SuffixArray sa(snew);
    
    int ans = 0;
    vector<int> wasteindices;
    for(int i=n;i<=2*n;i++){wasteindices.push_back(sa.c[i]);}
    for(int i=3*n + 1;i<=4*n;i++){wasteindices.push_back(sa.c[i]);}
    
    sort(all(wasteindices));
    for(auto i:wasteindices)
    {
        sa._lcp[i] = min(sa._lcp[i],sa._lcp[i-1]);
    }

    for(int i=0;i<n;i++){
        int x = max(n - (sa._lcp[sa.c[i] - 1]) , 0ll);
        debug(i,x);
        ans += max(n - (sa._lcp[sa.c[i] - 1]) , 0ll);
    }
