...
int32_t main(){
    ...
    for(;T--;)
    {
        ...
        vector<pair<int,int>> leftright(n,{-1,-1});
        stack<int> st;
        for(int i=0;i<n;i++)
        {
        ...
        for(int i=n-1;i>=0;i--)
        {
            ...
            if(st.size() != 0)
            {
                leftright[i].second = st.top();
            }
            st.push(i);
        }
        debug(leftright);
        vector<int> count(n);
        for(int i=0;i<n;i++)if(leftright[i].first != -1 and leftright[i].second != -1)
        {
            count[leftright[i].second] ++;
        }
        debug(count);
