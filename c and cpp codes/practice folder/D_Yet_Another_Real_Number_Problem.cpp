...
int32_t main()
{
    ...
    for(;T--;)
    {
        ...
        if(v[0]%2 == 0)
        {
            ...
            ihavetwo.emplace(v[0],cnt);
        }
        mint cursum = dp[0];
        for(int i=1;i<n;i++)
        {
            pair<int,int> cur = {v[i],0};
            while(cur.first%2 == 0)
            {
                cur.second++;
                cur.first>>=1;
            }
            while(ihavetwo.size() and (cur.second > 30 or ((cur.first << cur.second) > ihavetwo.top().first)))
            {
                auto [l,pow] = ihavetwo.top();
                ihavetwo.pop();
                cur.second += pow;
                cursum -= (mint)l *((mint)2)[pow];
                cursum += l;
            }
            cursum += (mint)cur.first * ((mint)2)[cur.second];
            dp[i] = cursum;
            if(cur.second > 0)
            {
                ihavetwo.emplace(cur);
            }
        }
        debug();
        arrPrint(dp);
