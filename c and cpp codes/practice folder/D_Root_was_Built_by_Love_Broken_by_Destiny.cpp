...
mint fact(int x) {mint ans = 1; for(int i=1;i<=x;++i) ans *= i; return ans;}

bool checkBipartite(vector<vector<int>>& edges, int n) {
    vector<int> color(n + 1, -1);
    auto dfs = [&](auto self, int x, int col) -> bool {
        color[x] = col;
        for(auto nbr : edges[x]) {
            if(color[nbr] == -1) self(self, nbr, col ^ 1);
...
signed main(){
    ...
    for(int TT = 1; TT <= T; ++TT)
    {
        ...
        if(!checkBipartite(edges, n)) {cout << 0 << endl; continue;}

        vector<int> cntnt(n + 1);
        mint mult = 1;
        bool fl = 1;
        for(int i=1;i<=n;++i) {
            int cnt = 0;
            for(auto nbr : edges[i]) if(edges[nbr].size() > 1) {++ cnt;}
            if(cnt > 2) {fl = 0; break;}
            cntnt[i] = cnt;
            if(cntnt[i] && edges[i].size() > 1) mult = 2;
        }
        bool fl2 = true;
        if(*max_element(all(cntnt)) == 2) {
            fl2 = false;
            for(int i=1;i<=n;++i) if(edges[i].size() > 1) if(cntnt[i] == 1) fl2 = true;
        }

        if(!fl || !fl2) {cout << 0 << endl; continue;}
        debug(cntnt);
        mint ans = 2 * mult;
        for(int i=1;i<=n;++i) {
            if(edges[i].size() > 1) ans *= fact(edges[i].size() - cntnt[i]);
        }
        cout << ans << endl;
    }
}